class FetchFollowingJob < ActiveJob::Base
  queue_as :twitter_api

  def perform(*args)
    # Do something later
    add_following(args[0])
  end
    ...
    def add_following (twitter_id)
      ...
      loop do
        ...
        i['ids'].each do |id|
            ...
            @twitter_relationship.follower = twitter_id
            @twitter_relationship.followed = id
        ...
        write_to_api_call_log("Following data was successfully retrived for twitter id: " + twitter_id, true, '/friends/ids', twitter_id, i['ids'].count)
